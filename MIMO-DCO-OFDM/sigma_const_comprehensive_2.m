...
sishma_original_2=  0.5820; % 122 64 qam
% 170
% 162 0.5062  64 qam
% 0.2486; % N=178 16QAM
sishma_original_1=0.2752; %   130 16 qam
...
b_dco=(N/2)*log2(M_qam_1); %%  bit in dco
b_dco_2=(b_dco+DCO_bit_extra);
b_dco_2/(log2(M_qam_2))
N_2=floor(b_dco_2/(log2(M_qam_2)));
N_half_2=N_2+1
phi_plot=[3:0.4:4.2];
        ...
        for etta=1:1:1
            ...
            for a=1:size(SNR,2)
                ...
                for jj=1:loop
                    ...
                    %  DCO modulation
                    dco_data=bi2de(reshape(b_2',log2(M_qam_1),N/2)');
                    dco_data_2=bi2de(reshape(b_3',log2(M_qam_2),N_2)');
                    in=qammod(dco_data,M_qam_1);
                    in_2=qammod(dco_data_2,M_qam_2);
                    out_en(2:N_t/2) =in;
                    out_en_2(2:N_half_2)=in_2;
                    out_en(1)=0;
                    out_en_2(1)=0;
                    out_en(N_t/2+1)=0;
                    out_en_2(N_half_2+1)=0;
                    I_mppm(ON_vect_Mppm)=I_ON;
                    for i=N_t/2+1:N_t
                        out_en(i)=conj(out_en(N_t+2-i));
                    end
                    for i=N_half_2+1:2*N_half_2
                    ...
                    f_out_dco=qamdemod(out_dco,M_qam_1);
                    f_out_dco_2=qamdemod(out_dco_2,M_qam_2);
                    bit_out_dco=de2bi(f_out_dco);
                    bit_out_dco_2=de2bi(f_out_dco_2);
                    reshaped_bit_dco=reshape(bit_out_dco',N/2*log2(M_qam_1),1);
                    reshaped_bit_dco_2=reshape(bit_out_dco_2',N_2*log2(M_qam_2),1);
...
for j=1:length(delta_plot)
    figure
    semilogy(SNR,squeeze(err_rate(j,1,:)),'b--');hold on;semilogy(SNR,squeeze(err_rate(j,2,:)),'r--')
     hold on;semilogy(SNR,squeeze(err_rate(j,3,:)),'k--')
     hold on;semilogy(SNR,squeeze(err_rate(j,4,:)),'g--')
    % hold on;semilogy(SNR,squeeze(err_rate(j,5,:)),'c--')
    hold on;semilogy(SNR,squeeze(err_rate_2(j,1,:)),'bv-');hold on;semilogy(SNR,squeeze(err_rate_2(j,2,:)),'rv-')
    hold on;semilogy(SNR,squeeze(err_rate_2(j,3,:)),'kv-')
    hold on;semilogy(SNR,squeeze(err_rate_2(j,4,:)),'gv-');
    % hold on;semilogy(SNR,squeeze(err_rate_2(j,5,:)),'cv-')
    legend('\lambda=2','\lambda=2.4','\lambda=2.8','\lambda=3.2','\lambda=2','\lambda=2.4','\lambda=2.8','\lambda=3.2' )
...
for j=1:length(delta_plot)
    figure
    semilogy(dB,squeeze(PAPR_CCDF(:,1,j)),'b--');hold on;semilogy(dB,squeeze(PAPR_CCDF(:,2,j)),'r--');
    hold on;semilogy(dB,squeeze(PAPR_CCDF(:,3,j)),'k--')
    hold on;semilogy(dB,squeeze(PAPR_CCDF(:,4,j)),'g--')
    % ;hold on;semilogy(dB,squeeze(PAPR_CCDF(:,5,j)),'c--');hold on;
    hold on;semilogy(dB,squeeze(PAPR_CCDF_2(:,1,j)),'bv-');hold on;semilogy(dB,squeeze(PAPR_CCDF_2(:,2,j)),'rv-');
    hold on;semilogy(dB,squeeze(PAPR_CCDF_2(:,3,j)),'kv-')
